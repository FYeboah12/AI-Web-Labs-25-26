...
def cond_prob(arg, graph): # P(A|B) = P(A,B) / (P(A,B) + P(~A,B))
    ...
    if "," in right: 
        ...
        if "~g" in right:
            ...
            g_val = "g"
        if g_val is not None: #conditionally independent case
...
def main():
    ...
    probability = None
    while probability != "stop":
        probability = input("Calculate probability. Type your desired probability as specified in the user guide. Type stop to stop.\n")
        if probability != "stop":
