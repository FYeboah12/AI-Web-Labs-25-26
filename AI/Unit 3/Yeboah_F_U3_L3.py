...
#8x8
   def best_strategy(self, board, color):
      ...
      #randomly pick a move from keys, random index
      best_move = moves[random_index] #turn into col,row
      return [best_move // self.y_max, best_move % self.y_max], 0
...
class Best_AI_bot: #done?
   ...
   def evaluate(self, board, color, possible_moves):
    # returns the utility value
      ...
      global static_weights
      hash = self.make_key(board,color)
      sum = 0
      for move in possible_moves:
         col, row = hash[move]
...
class Alpha_beta_AI_bot: #done
   ...
   def evaluate(self, board, color, possible_moves):
    # returns the utility value
      #cur_color possible moves - opp_color possible moves
...
class Minimax_AI_bot: #done, use deep copy
   ...
   def make_key(self, board, color):
    # hashes the board
      ...
      hashes = []
      # for i in range(9):
      #    hashes.append([i + 8 for i in range()])
      return 1
