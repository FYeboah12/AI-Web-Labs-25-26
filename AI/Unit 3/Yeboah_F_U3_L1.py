...
def utility(turn, tc, state):
   #turn is x or o
   #tc is winning conditions
   # return 1 (turn wins), -1 (turn loses), or 0 (tie) 
   fill = []
   for possible_win in tc:
      fill.append([state[i] for i in possible_win])
   for possible_win in fill:   
    if possible_win.count(turn) == 3: return 1
    if possible_win.count(turn) == 0: return -1
   return 0   
